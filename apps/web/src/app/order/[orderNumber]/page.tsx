...
interface Props {
  ...
  /** `t` is the order's view token; `email` is the older form of the same link
   *  and still accepted so receipts already sent keep working. `new=1` marks
   *  the arrival from checkout — the only visit that reports a Purchase. */
  searchParams: Promise<{ email?: string; t?: string; new?: string }>;
}
  ...
  try {
    const qs = auth.token
      ...
      : auth.email
        ...
        : "";
    const res = await fetch(`${API}/orders/${orderNumber}${qs}`, {
      cache: "no-store",
    });
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10 sm:px-6">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-paper-dim">
            Order <strong className="text-paper">{order.orderNumber}</strong> ·
        ...
        <section className="mt-8 border border-paper/10">
          ...
          {order.items.map((i) => (
            <div
              key={i.id}
              className="flex items-center gap-3 border-b border-paper/5 px-4 py-3 last:border-0"
            >
